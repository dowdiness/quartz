...
publish: false
tags: [" "]
aliases: [Structure editorとは何か？, 無題のファイル]
created: 2026-01-03T14:08:04+09:00
modified: 2026-01-03T14:51:21+09:00
---
...
Martin Fowlerが提唱した**Projectional Editing（投影型編集）**は、この考えをさらに拡張したものです。

- **ソースコード（真のモデル）：** 永続化されたデータ構造（例：XML, JSON, 独自のバイナリ）。
- **投影（プロジェクション）：** 開発者が目にする画面（テキスト形式、フローチャート形式、数式形式など）。

開発者は自分の好みの「投影」を選択して編集しますが、操作は常に背後にある共通のモデルに対して行われます。

プログラムが持ちうる本質的なモデルは1つしかないが、それを表現する形は複数あるようなDSLを想像してみましょう。

Projectional Editingという言葉もあります。システムを表現するための媒体が複数存在していて、その編集をするための媒体を選べるという発想です。

例えばプログラムはそのままの形では実行できません。コンパイラによって意味論的に同じなままの実行可能な形へと変換されることにより実行できます。このときプログラムには編集のための表現と、実行するための表現の複数が存在することになります。


![Martin Fowlerの説明](https://www.martinfowler.com/articles/workbench.gif)
...
異なる表現形式（例：テキスト形式 A と グラフィカル形式 B）の間で編集を行う際、「形式 A での変更が、形式 B においても意味的に等価であること」を保証しなければなりません。ここで登場するのが双模倣性です。
...
構造的編集を実現しようとすると、従来のテキストエディタでは発生しなかった（あるいは無視されていた）以下の問題に直面します。
...
このトピックを深掘りする場合、以下の概念を追記するとさらに説得力が増します。

- **Hazel:** 「穴あき」の状態（Typed Holes）を許容する関数型プログラミング環境の研究。上記の Problem 1-3 への一つの回答です。
- **JetBrains MPS:** Projectional Editing を実用化している言語ワークベンチの代表例。
- **LSP (Language Server Protocol):** テキストと構造（セマンティクス）の橋渡しをする現代的なデファクトスタンダード。
