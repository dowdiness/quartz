---
title: Hindley–Milner type system
publish: false
tags: [" "]
aliases: [Untitled]
created: 2025-05-05T15:32:35+09:00
modified: 2025-12-28T20:03:58+09:00
---

# Hindley–Milner type system

[Wiki](https://en.wikipedia.org/wiki/Hindley%E2%80%93Milner_type_system)

## Blog

[型推論器の実装①　Hindley-Milner型システム](https://zenn.dev/ksrk/articles/5e4a6858c43d6f)
日本語での貴重な解説です。Algorithm WだけでなくAlgorithm Mと副作用のある最適化したアルゴリズムの紹介もある。

[Understanding Algorithm W](https://jeremymikkola.com/posts/2018_03_25_understanding_algorithm_w.html)
## Papers

[The Hindley-Milner Type Inference Algorithm](https://steshaw.org/hm/hindley-milner.pdf)

## Videos

 [Haskell Type System (YouTube playlist)](https://www.youtube.com/watch?v=nAlyU4OPa40&list=PLLMfIaSUuqCPpR8N5uXlp0s4Byts2v51R)
 Algorithm Wの詳しい解説が聞けます

[Type systems: Lambda calculus to Hindley-Milner](https://www.youtube.com/watch?v=b5VhYkvOk30&list=PLoyEIY-nZq_uipRkxG79uzAgfqDuHzot-)
[hindley-milner-typescript-minimal](https://github.com/domdomegg/hindley-milner-typescript-minimal): 上記のシリーズのTypeScriptでのHindley–Milnerの実装
## 実装

[Algorithm-W-Step-By-Step](https://github.com/wh5a/Algorithm-W-Step-By-Step)